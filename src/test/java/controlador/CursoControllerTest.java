...
public class CursoControllerTest {
	...
	void crearCurso_debe_incrementar_lista() {
		...
		boolean creado = controller.crearCurso("CS104", "Nuevo Curso Test", "Lun/Mie 10:00-11:30", "Sala 104", null);
	...
	void crearCurso_duplicado_debe_fallar() {
		// Crear primer curso
		boolean primerCreacion = controller.crearCurso("CS105", "Curso Test 1", "Horario 1", "Aula 1", null);
		assertTrue(primerCreacion);

		// Intentar crear curso con mismo código
		boolean segundaCreacion = controller.crearCurso("CS105", "Curso Test 2", "Horario 2", "Aula 2", null);
	...
	void actualizarCurso_debe_modificar_correctamente() {
		...
		// Actualizar curso
		boolean actualizado = controller.actualizarCurso(curso.getId(), "CS106", "Curso Actualizado", "Nuevo Horario",
				"Nueva Aula", "P001");
	...
	void buscarCursos_debe_encontrar_por_nombre() {
		...
		boolean encontrado = resultados.stream().anyMatch(c -> c.getNombre().contains("Sistemas"));
	...
	void buscarCursos_debe_encontrar_por_codigo() {
		...
		boolean encontrado = resultados.stream().anyMatch(c -> c.getCodigo().equals("CS101"));
		assertTrue(encontrado);
	}
	...
	void buscarCursos_debe_encontrar_por_horario() {
		...
		boolean encontrado = resultados.stream().anyMatch(c -> c.getHorario().contains("Lun/Mie"));
		assertTrue(encontrado);
	}
	...
	@Test
	void actualizarCurso_con_codigo_duplicado_debe_fallar() {
		// Crear dos cursos
		controller.crearCurso("TEST201", "Curso A", "Horario A", "Aula A", null);
		controller.crearCurso("TEST202", "Curso B", "Horario B", "Aula B", null);

		Curso cursoA = controller.obtenerCursoPorCodigo("TEST201");
		Curso cursoB = controller.obtenerCursoPorCodigo("TEST202");

		assertNotNull(cursoA);
		assertNotNull(cursoB);

		// Intentar cambiar código de cursoB al mismo que cursoA
		boolean resultado = controller.actualizarCurso(cursoB.getId(), "TEST201", // Mismo código que cursoA
				"Curso B Actualizado", "Nuevo Horario", "Nueva Aula", null);

		assertFalse(resultado, "No debería permitir código duplicado");

		// Limpiar
		controller.eliminarCurso(cursoA.getId());
		controller.eliminarCurso(cursoB.getId());
	}

	@Test
	void obtenerCursoPorId_con_id_invalido_debe_retornar_null() {
		Curso resultado = controller.obtenerCursoPorId("ID_INEXISTENTE_12345");
		assertNull(resultado);
	}

	@Test
	void eliminarCurso_con_id_invalido_debe_fallar() {
		boolean resultado = controller.eliminarCurso("ID_INEXISTENTE_12345");
